...
async function drawChart(data){
    ...
    const drawLine = settings => {
        // Init settings
        var dataFilter = settings['dataFilter']
        var trendLine = settings['trendLine']
        var update = settings['update']
        var drawTrades = settings['drawTrades']

        // 4. Create Scales
        const yScale = d3.scaleLinear()
            .domain(d3.extent(dataFilter, yAccessor))
            .range([dimensions.boundedHeight, 0])
            .nice()

        const xScale = d3.scaleTime()
            .domain(d3.extent(dataFilter, xAccessor))
            .range([0, dimensions.boundedWidth])

        // 5. Draw Data
        const exitTransition = d3.transition().duration(600)
        const updateTransition = d3.transition().duration(600).ease(d3.easeCircleOut) 
        ...
        bounds.select(".area").remove()
        bounds.select(".closePrice").remove()
        bounds.selectAll(".trade").remove()
        bounds.select(".yGrid").remove()
        bounds.select(".yAxis").remove()
        bounds.select(".xAxis").remove()
        ...
        const line = bounds.append('path')
            .transition(updateTransition)
                .attr('class', 'closePrice')
                .attr('d', lineGenerator(dataFilter))
                .attr('fill', trendLine)
                .attr('stroke', '#53dd6c')
                .attr('stroke-width', 1)
        ...
        function tradeLine(trade, index){
            ...
            const tradePrice = bounds.append("line")
                .attr('class', 'trade')
                .attr("x1", xScale(d3.min(dataFilter, d => d.date)))
                .attr("x2", xScale(d3.max(dataFilter, d => d.date)))
    ...
    drawLine({'dataFilter': data, 'trendLine': 'none', 'drawTrades': false})
    ...
    function clickTime(){
        ...
        current_data = filterData(data, months)
        drawLine({'dataFilter': current_data, 'trendLine': trend, 'drawTrades': tradeLines})
    }

    function clickTimeIn(){
    ...
    function resetChart(){
        ...
        $('#reset_chart').removeClass('active')
        $('#10000').addClass('active')
        trend = 'none'
        tradeLines = false
        current_data = filterData(data, '10000')
        drawLine({'dataFilter': current_data, 'trendLine': trend, 'drawTrades': tradeLines})
    }

    const timeLines = d3.selectAll('.priceChart').select(function(){
        this.addEventListener('click', clickTime)
    })

    const timeIn = d3.select('#timeIn').select(function(){
        this.addEventListener('click', clickTimeIn)
    })

    const tradesBtn = d3.select('#show_trades').select(function(){
        this.addEventListener('click', clickLineAdd)
    })

    const trendBtn = d3.select('#show_trend').select(function(){
        this.addEventListener('click', clickLineAdd)
    })

    const resetBtn = d3.select('#reset_chart').select(function(){
        this.addEventListener('click', resetChart)
    })
}
...
var current_data = parseDate(price_data)
