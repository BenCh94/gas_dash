...
class StockUpdate():
  def __init__(self):
    ...
    self.trades = [trade_1, trade_2, trade_3]

  def trade_charts(self):
    # Loop through trades and collate prices and amounts incrementing buys then detracting sells
    # need trades to be ordered by date
  ...
  def apply_price_chart(self, trades_chart):
    ...
    trades_chart['bench_close'] = benchmark_prices.array
...
trades_chart = StockUpdate().trade_charts()
full_price_chart = StockUpdate().apply_price_chart(trades_chart)
print(full_price_chart)
